...
for (p = proc; p < &proc[NPROC]; p++)
    ...
    for(p = proc;p < &proc[NPROC];p++){
      ...
      if(p->state == RUNNABLE){
        if(min_proc == 0)
          min_proc = p;
        else if(min_proc->ctime > p->ctime){
          min_proc = p;
          continue;
        }
      }
...
struct proc *next_process = 0;
    // min priority can now be 101 (or anything > 100)
    uint min = 101; 
    for (p = proc; p < &proc[NPROC]; p++)
    {
      acquire(&p->lock);
      if (p->state == RUNNABLE)
      {
        ...
        // Adjustment range : +5 (CPU-bound) to -5 (I/O-bound)
        ...
        if (next_process == 0)
        {
          ...
          next_process = p;
          // (BUG FIX: continue was in the wrong place)
        }
    ...
    if (next_process != 0)
    {
      // (We are still holding the lock on next_process)
      if (p->state == RUNNABLE)
      {
        ...
        // Handle race condition where process state changed
        release(&p->lock);
      }
    }
#elif defined MLFQ
    // --- 1. GLOBAL LOCK ---
    // We MUST acquire a global lock to protect the shared 'mlfq' queues
    // during the entire aging, re-queuing, and selection process.
    acquire(&proc->lock);

    // --- 2. AGING LOOP ---
    // Use the function-scoped 'p' (assuming it's declared at the
    // top of scheduler()) for iteration.
    ...
    // --- 3. RE-QUEUING LOOP ---
    // Also use the function-scoped 'p' here.
    for(struct proc* p = proc; p < &proc[NPROC]; p++){
    ...
    // --- 4. SELECTION LOOP ---
    // Define 'next_process' as requested, to hold our chosen process.
    struct proc *next_process = 0; 
    for(int level = 0; level < NMLFQ; level++){
      while(size(&mlfq[level])){
        // Use 'p' as a temporary variable to check the front
        p = front(&mlfq[level]);
        
        // --- FIX: Added missing pop ---
        // You must remove the process from the queue
        pop(&mlfq[level]); 
        p->in_queue = 0;
        
        if(p->state == RUNNABLE){
          // Found our process. Assign it to next_process.
          next_process = p; 
          p->enter_ticks = ticks;
      ...
      if(next_process != 0){
        // If we found a process, break from the 'for' loop
        break;
      }
    }

    // --- 5. EXECUTION BLOCK (Similar to PBS) ---
    // Check if we successfully found a process to run.
    if(next_process != 0){
      // We found one. Assign it to 'p' so the
      ...
      // Release the global lock *before* acquiring the
      // specific process lock to prevent deadlock.
      release(&proc->lock);
