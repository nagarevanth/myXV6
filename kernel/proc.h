...
struct proc
{
  ...
  uint sleep_start;
  uint reset_niceness; // for pbs 
  uint sleeping_ticks; // for pbs
  uint running_ticks; // for pbs
  uint level;         // indicates the level in which proc is in MLFQ
  uint in_queue;       
  uint change_queue;
  uint enter_ticks;   // stores the time a proc since it entered a queue -> useful for aging 
